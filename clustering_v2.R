...
metrics <- rbindlist(
  pbmclapply(
    ks,
    function(k) {
      cl <- cutree(hc, k)                  # cluster labels

      # --- internal validation metrics ---------------------------------------
      sil <- silhouette(cl, d)             # per‑point silhouette widths
      cs  <- cluster.stats(d, cl)          # many extra indices (fpc pkg)

      data.table(
        k                 = k,
        avg_silhouette    = mean(sil[, 3]),
        dunn              = cs$dunn,
        calinski_harabaz  = cs$ch,         # higher ⇒ better separation/compact.
        entropy           = cs$entropy     # lower ⇒ purer clusters
      )
    },
    mc.cores = min(length(ks), parallel::detectCores() - 1L)
  )
)
metrics
